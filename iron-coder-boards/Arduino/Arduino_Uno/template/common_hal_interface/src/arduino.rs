...
#[cfg(any(feature = "arduino-leonardo", feature = "arduino-micro", feature = "pro-micro"))]
#[macro_export]
macro_rules! setup_spi {
    ($pins:expr) => {{  
        {            
...
#[macro_export]
macro_rules! setup_i2c_instance {
    ($dp:expr, $pins:expr, $freq:expr) => {
        {
            let (sda, scl) = setup_i2c!($pins);
            arduino_hal::I2c::new($dp.TWI, sda, scl, $freq)
        }
    };
}
#[macro_export]
macro_rules! setup_spi_instance {
        ($dp:expr, $pins:expr) => {{
